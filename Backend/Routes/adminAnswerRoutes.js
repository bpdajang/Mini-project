...
router.post("/create", auth, async (req, res) => {
  try {
    ...
    let { userId, infoReportId, answerText } = req.body;

    // If userId is not provided or is invalid, try to get it from the infoReport
    if (!userId || userId === "undefined") {
      const infoReport = await UserInfo.findById(infoReportId);
      if (infoReport && infoReport.userId) {
        userId = infoReport.userId;
      } else {
        return res
          .status(400)
          .json({ message: "Unable to determine userId from infoReport" });
      }
    }
