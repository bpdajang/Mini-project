...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/userinfo/user/${currentUser._id}`, {
      credentials: "include",
    })
      .then((res) => {
    ...
    const fetchUserAnswers = async () => {
      try {
        const res = await fetch(
          `http://localhost:5000/api/adminanswers/user/${currentUser._id}`,
          { credentials: "include" }
        );
    ...
    <div className="flex min-h-screen">
      ...
      <div className="flex-1 overflow-x-hidden">
        <div className="container mx-auto px-4 py-8">
          <div className="md:hidden mb-6 absolute top-10">
            <button
              ref={menuButtonRef}
              onClick={toggleSidebar}
              className="bg-white text-gray-500 px-4 py-2 rounded-lg flex items-center "
            >
              <svg
                ...
                <div className="space-y-6">
                  {messageThreads.length === 0 ? (
                    ...
                    messageThreads.map((thread) => (
                      <div key={thread.report._id} className="mb-8">
